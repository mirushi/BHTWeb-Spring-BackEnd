...
create sequence post_view_avg_sequence start with 1 increment by 50;
...
create table doc_file_upload (id uuid not null, created_dtm timestamp not null, downloadurl varchar(255) not null, file_name varchar(255) not null, file_size bigint not null, rank integer not null, remoteid varchar(255) not null, thumbnailurl varchar(255) not null, doc_id bigint, uploader_id uuid not null, primary key (id));
...
create table exercise_answer (id bigint not null, content varchar(255), is_correct boolean, rank integer, version smallint not null, question_id bigint, primary key (id));
...
create table post_view_avg (id bigint not null, past_view bigint not null, recent_avg float(8) not null, sqr_past_view bigint not null, post_id bigint not null, primary key (id));
create table report_reason (id bigint not null, reason varchar(255), primary key (id));
create table report_reason_feedback (feedback varchar(255), post_report bigint not null, user_id uuid not null, primary key (post_report, user_id));
...
create table user_website (id uuid not null, about_me varchar(255) not null, avatarurl varchar(255) not null, display_name varchar(255) not null, email varchar(255) not null, name varchar(255) not null, reputation_score bigint not null, version smallint not null, primary key (id));
...
alter table post_view_avg add constraint FKedbu5v4eybxhp18tdufqr8nty foreign key (post_id) references post;
alter table report_reason_feedback add constraint FKj3hbujaxl2fut840x1xrd9cqc foreign key (post_report) references post_report;
alter table report_reason_feedback add constraint FKsn9ue13y8a4pmebrj2vqscxvl foreign key (user_id) references user_website;
